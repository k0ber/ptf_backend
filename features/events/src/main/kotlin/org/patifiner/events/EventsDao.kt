...
interface EventsDao {
    suspend fun create(creatorId: Long, request: CreateEventRequest): EventDto
    suspend fun update(eventId: Long, request: UpdateEventRequest): EventDto
    suspend fun delete(eventId: Long)
    suspend fun getById(eventId: Long): EventEntity
    suspend fun setParticipantStatus(eventId: Long, userId: Long, status: ParticipantStatus): EventDto
}

internal class ExposedEventsDao : EventsDao {
    override suspend fun create(creatorId: Long, request: CreateEventRequest): EventDto =
    ...
    override suspend fun getById(eventId: Long): EventEntity =
        newSuspendedTransaction(Dispatchers.IO) {
            EventEntity.findById(eventId) ?: throw EventNotFoundException(eventId)
        }

    override suspend fun update(eventId: Long, request: UpdateEventRequest): EventDto =
        newSuspendedTransaction(Dispatchers.IO) {
            ...
            request.topicIds?.let { ids -> 
                event.topics = TopicEntity.find { TopicsTable.id inList ids }
            }
    ...
    override suspend fun delete(eventId: Long) =
        newSuspendedTransaction(Dispatchers.IO) {
            EventEntity.findById(eventId)?.delete() ?: throw EventNotFoundException(eventId)
        }

    override suspend fun setParticipantStatus(eventId: Long, userId: Long, status: ParticipantStatus): EventDto =
        newSuspendedTransaction(Dispatchers.IO) {
