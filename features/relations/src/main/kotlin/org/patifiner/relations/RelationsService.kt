...
internal class RelationsService(private val relationsDao: RelationsDao) {

    suspend fun getMyRelations(userId: Long): List<UserRelationDto> = newSuspendedTransaction(Dispatchers.IO) {
        relationsDao.findUserRelations(userId).map { it.toDto(userId) }
    }

    suspend fun blockUser(currentUserId: Long, targetUserId: Long): UserRelationDto = newSuspendedTransaction(Dispatchers.IO) {
    ...
    suspend fun sendInvite(fromUserId: Long, toUserId: Long): UserRelationDto = newSuspendedTransaction(Dispatchers.IO) {
        ...
        newRelation.toDto(fromUserId)
    }

    suspend fun acceptInvite(currentUserId: Long, targetUserId: Long): UserRelationDto = newSuspendedTransaction(Dispatchers.IO) {
