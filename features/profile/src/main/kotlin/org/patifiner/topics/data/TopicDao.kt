...
class ExposedTopicDao : TopicDao {
    ...
    override suspend fun addUserTopics(userId: Long, requests: List<AddUserTopicRequest>): Set<UserTopicDto> =
        newSuspendedTransaction(Dispatchers.IO) {
            val user = UserEntity.findById(userId) ?: throw UserException.UserNotFoundByIdException(userId)

            requests.map { req ->
                val topic = TopicEntity.findById(req.topicId) ?: throw TopicsException.TopicNotFoundException(req.topicId)
    ...
    override suspend fun removeUserTopics(userId: Long, topicIds: List<Long>): Long =
        newSuspendedTransaction(Dispatchers.IO) {
            val toDelete = UserTopicEntity.find {
                (UserTopicsTable.user eq EntityID(userId, UsersTable)) and
    ...
    override suspend fun getUserTopics(userId: Long): Set<UserTopicDto> =
        newSuspendedTransaction(Dispatchers.IO) {
            UserTopicEntity.find { UserTopicsTable.user eq EntityID(userId, UsersTable) }
                .map { it.toDto() }
                .toSet()
        }
